...
function usage() {
	...
	println("--------------\n");
	println(
		"st                          Stalk  person usage: ./chrome.js st idvorkin",
	);
	println(
		"list                        List all open tabs in all Chrome windows          usage: ./chrome.js list",
	);
	println(
		"dedup                       Close duplicate tabs                              usage: ./chrome.js dedup",
	);
	println(
		"close <winIdx,tabIdx>       Close a specific tab in a specific window         usage: ./chrome.js close 0,13",
	);
	println(
		'close --title <string(s)>   Close all tabs with titles containing strings     usage: ./chrome.js close --title Inbox "iphone - apple"',
	);
	println(
		"close --url <string(s)>     Close all tabs with URLs containing strings       usage: ./chrome.js close --url mail.google apple",
	);
	println(
		"focus <winIdx,tabIdx>       Focus on a specific tab in a specific window      usage: ./chrome.js focus 0,13",
	);
	println(
		"--ui                        If set, use Chrome to show messages               usage  ./chrome.js close --title inbox --ui",
	);
	println(
		'--yes                       If set, all questions will be answered with "y"    usage  ./chrome.js close --title inbox --yes',
	);
	$.exit(1);
}
...
function chromeControl(argv) {
	...
	const uiFlagIdx = argv.indexOf("--ui");
	if (uiFlagIdx > -1) {
		MODE = MODE_UI;
	...
	const yesFlagIdx = argv.indexOf("--yes");
	if (yesFlagIdx > -1) {
		MODE = MODE_YES;
...
function closeTab(arg) {
	const { winIdx, tabIdx } = parseWinTabIdx(arg);

	const tabToClose = chrome.windows[winIdx].tabs[tabIdx];
...
function closeByKeyword(cmd, keywords) {
	if (cmd === "--title") {
		getProperty = (tab) => tab.title();
	} else if (cmd === "--url") {
		getProperty = (tab) => tab.url();
	} else {
		usage();
	}
...
function parseWinTabIdx(arg) {
	...
	const winIdx = Number.parseInt(s[0]);
	const tabIdx = Number.parseInt(s[1]);
